...
def pretty_print_sample(samples: list[dict]) -> str:
    ...
    for sample in samples:
        ...
        string += f"Size (bytes): {sample['size']}\n"
        string += f"State: {sample['verdict']['job']['state']}"
        for score in sample["verdict"]["scores"].items():
            string += f"Score status, certainty: {score[0]} ({score[1]})\n"
        string += "Categories:"
        for category in sample["verdict"]["categories"].items():
            string += f" {category[0]} ({category[1]})" if category[1] >= 0 else ""
        string += "\n"
        if sample["verdict"]["comment"]:
            string += f"Comment: {sample['verdict']['comment']}"
    return string
